...
def precompute_rope2d_freqs_grid(dim, dynamic_resolution_h_w, rope2d_normalized_by_hw, pad_to_multiplier=1, max_height=2048 // 16, max_width=2048 // 16, base=10000.0, device=None, scaling_factor=1.0):
    required_max_height = max(
        h
        for ratio_cfg in dynamic_resolution_h_w.values()
        for preset_cfg in ratio_cfg.values()
        for _, h, _ in preset_cfg['scales']
    )
    required_max_width = max(
        w
        for ratio_cfg in dynamic_resolution_h_w.values()
        for preset_cfg in ratio_cfg.values()
        for _, _, w in preset_cfg['scales']
    )
    max_height = max(max_height, required_max_height)
    max_width = max(max_width, required_max_width)
